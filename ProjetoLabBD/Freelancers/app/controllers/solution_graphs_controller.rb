class SolutionGraphsController < ApplicationController
  ...
  def index
    @service_graphs = SolutionGraph.all
  end
  ...
  # SolutionGraph.create({:login => "login", :name => "name"}).save
  # SolutionGraph.all = array de SolutionGraphs
  # SolutionGraph.followers << @service_graph2

  # GET /service_graphs/new
  def new
    @service_graph = SolutionGraph.new
  end
  ...
  private
    ...
    def service_graph_params
      params.require(:service_graph).permit(:code, :desc)
    end
end
